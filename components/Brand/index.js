...
export default function Brand() {
  return (
    <div className="my-3.5 lg:mt-8 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-8 gap-3.5 mx-3.5 lg:m-0 lg:p-4 lg:border-2 border-[#C4C4C4] relative">
      ...
      {brands?.map((brand, index) => (
        <div key={index} className="w-full border-2 border-[#C4C4C4] lg:border-none">
          <img className="w-full h-16 object-cover lg:h-auto" key={index} src={brand} />
        </div>
      ))}
    </div>
  )
